...
current_broker_port: Optional[int] = None
...
def connect_to_broker(broker: str = "localhost", port: int = 1883, topic: str = "lora/+/up") -> None:
    ...
    """
    global mqtt_client, current_broker_ip, current_broker_port

    # Normalize and persist broker config
    broker = str(broker or "localhost").strip()
    port = int(port or 1883)
    save_broker_config(broker, port, topic)

    # If we already have a client but the broker/port changed, cleanly reconnect
    if mqtt_client is not None and (broker != current_broker_ip or port != (current_broker_port or port)):
        try:
            try:
    ...
    if mqtt_client is None:
        ...
        # Same broker/port, just update subscription/topic
