...
def get_app_root() -> str:
    """Directory containing the shipped static assets (repo root when in static/py; _MEIPASS when frozen)."""
    if is_frozen():
        return str(getattr(sys, "_MEIPASS"))
    # When in static/py, __file__ is .../static/py/app_paths.py; repo root is parent of static
...
def get_data_root() -> str:
    ...
    # Running from source: use repo root (parent of static/py when this module lives there)
    src_root = get_app_root()
...
def get_custom_decoders_dir() -> str:
    p = os.path.join(get_data_root(), "static", "js", "decoders", "custom")
    os.makedirs(p, exist_ok=True)
    return p
